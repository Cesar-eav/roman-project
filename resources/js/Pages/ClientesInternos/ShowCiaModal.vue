<template>
  <Modal :show="show" :max-width="maxWidth" :closeable="closeable" @close="close">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-slate-900 mx-4 text-sm ">
      ...
      <button class="bg-orange-500 text-white px-4 py-2 rounded" @click="addEjecutiva">Agregar más campos</button>
...
export default {
  ...
  emits: ['update-ejecutivas-data', 'close'], // Añade 'update-ejecutivas-data'
  data() {
    return {
      showEjecutiva: false,
      ejecutivas: [],
  ...
  methods: {
    ...
    saveChanges(id) {
      ...
      if (index !== -1) {
        ...
        axios.post("/edit-save-ejecutiva-modal", {
          id: id,
    ...
    deleteEjecutiva(id) {
      ...
      const ejecutivasBackup = [...this.ejecutivas];

      this.ejecutivas = this.ejecutivas.filter(ejecutiva => ejecutiva.id !== id);



      // Esta línea utiliza el método findIndex de JavaScript para encontrar el índice del primer elemento 
      // en el array this.companies.ejecutivas que tiene un id igual al id proporcionado. Si se encuentra
      //  un elemento con ese id, findIndex devuelve el índice de ese elemento. Si no se encuentra, devuelve -1.

      // ejecutiva => ejecutiva.id === id: 
      // Es una función flecha que toma cada ejecutiva en el array y verifica si 
      // su id es igual al id proporcionado. Si encuentra una coincidencia, devuelve el índice de esa ejecutiva.

      //Devuelve una CONINCIDENCIA.  Del ARRAY this.companies.ejectuvias, designo con el nombre EJECUTIVA
      // al elemento que conincidad con el ID que viene por parametro con el ID de las EJECUTIVAS DEL ARRAY.
      const index = this.companies.ejecutivas.findIndex(ejecutiva => ejecutiva.id === id);

      if (index !== -1) {

        this.companies.ejecutivas.splice(index, 1);


        axios.delete("/delete/ejecutiva-modal/" + id)
          .then(response => {
            console.log("Ejecutiva eliminada correctamente:", response.data);
            const updatedEjecutivasData = this.ejecutivasData.filter(ejecutiva => ejecutiva.id !== id);
            console.log("Emitiendo evento update-ejecutivas-data con:", updatedEjecutivasData);
            this.$emit('update-ejecutivas-data', updatedEjecutivasData);
          })
          .catch(error => {
            console.error("Error al eliminar la ejecutiva:", error);
            this.ejecutivas = ejecutivasBackup;
          });
      }
    },
  close() {
